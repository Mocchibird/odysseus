...
async function _fetchDependencies() {
  ...
  try {
    ...
    async function _rebuildLlamaCpp(updateSource = false, statusEl = null) {
      ...
      const _rebuildMsg = `${action} on ${where}?\n\n${detail}`;
      const _rebuildOk = (uiModule && uiModule.styledConfirm)
        ? await uiModule.styledConfirm(_rebuildMsg, { confirmText: updateSource ? 'Update' : 'Rebuild' })
        : confirm(_rebuildMsg);
...
function _wireTabEvents(body) {
  ...
  if (rebuildBtn && !rebuildBtn._wired) {
    ...
    rebuildBtn.addEventListener('click', async () => {
      ...
      const host = _envState.remoteHost || '';
      const where = host || 'this server';
      // Upstream refactored the rebuild flow into the shared _rebuildLlamaCpp
      // helper (also driven from the dep-menu and window._cookbookRebuildLlamaCpp);
      // route through it instead of duplicating the inline body. The fork's
      // styledConfirm UX is preserved inside that helper.
...
function _renderRecipes() {
  ...
  // Chevron on the RIGHT \u2014 collapsed points right, expanded rotates to a down
  // chevron (handled by existing toggle CSS). SVG (not a dingbat) per the
  // no-Unicode-glyph rule.
  html += `<span id="cookbook-hf-latest-arrow" style="display:inline-block;transition:transform 0.15s;pointer-events:none;opacity:0.6;transform:rotate(-90deg);"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" style="display:block;"><polyline points="6 9 12 15 18 9"/></svg></span>`;
  html += `</button>`;
  html += `</div>`;
