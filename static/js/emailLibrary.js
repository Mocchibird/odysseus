...
// Last account the user actually looked at — the prewarm chooser prefers it so a
// cold open paints the inbox they expect rather than whichever account sorts first.
...
async function _toggleCardPreview(card, em) {
  ...
  try {
    ...
    const res = await _emailReadFetch(`${API_BASE}/api/email/read/${encodeURIComponent(uidAtStart)}?folder=${encodeURIComponent(folderAtStart)}${accountQueryAtStart}&mark_seen=true`);
    // A deadline hit resolves to a synthetic {error}, which the data.error branch
    // below reports; a real HTTP failure with no JSON body throws to the catch.
    const data = await res.json().catch(() => null);
...
function _safeRenderEmailBody(data) {
  try {
    ...
    if (data?.body_html) return _prepareEmailInlineImages(_sanitizeHtml(_rewriteInlineEmailImages(data.body_html, data)));
...
async function _ensureEmailAttachmentData(uid, folder, data, knownHasAttachments = false) {
  ...
  try {
    const res = await _emailReadFetch(`${API_BASE}/api/email/read/${encodeURIComponent(uid)}?folder=${encodeURIComponent(folder || 'INBOX')}${_acct()}&mark_seen=false&full=1`);
...
function _showCardMenu(em, anchor) {
  ...
  // Mark Read / Unread — single-uid version of the bulk action's read/unread
  // branch (same endpoint + read-state sync + unread-badge refresh). Icon
  // pair matches _showBulkActionsMenu.
  ...
  actions.push({
    ...
    icon: em.is_read ? _unreadIcon : _readIco,
    action: async () => {
      const target = !em.is_read;
      try {
        const res = await fetch(`${API_BASE}/api/email/${target ? 'mark-read' : 'mark-unread'}/${em.uid}?folder=${encodeURIComponent(state._libFolder)}${_acct()}`, { method: 'POST' });
        let data = null;
        ...
        em.is_read = target;
        _syncEmailReadState(em.uid, target);
        _refreshUnreadBadge();
      } catch (e) {
