...
function _cardHtml(img) {
  ...
  return `
      <div class="gallery-card" data-id="${_esc(img.id)}">
        ...
        <button class="gallery-fav-btn${favCls}" data-id="${_esc(img.id)}" title="Favorite" aria-label="Favorite">&#9829;</button>
        <button class="gallery-dl-btn" data-id="${_esc(img.id)}" data-url="${_esc(img.url)}" data-filename="${_esc(img.filename || '')}" title="Download" aria-label="Download">
        ...
        ${_isVideoUrl(img.url) ? `<span class="gallery-card-play" aria-hidden="true">
...
export function openGallery() {
  ...
  if (visionLink) {
    visionLink.addEventListener('click', (e) => {
      e.preventDefault();
      import('./settings.js?v=563').then(m => {
        m.open('ai');
  ...
  function _setSelectMode(on) {
    ...
    if (!on) document.querySelectorAll('.gallery-bulk-menu').forEach(dismissOrRemove);
    _updateBulkCount();
  }
