...
document.addEventListener('click', function(e) {
  ...
  if (kind === 'session') {
    ...
    import('./notes.js?v=526').then(mod => {
    ...
    if (
      ...
      for (let roundNum = firstRound; roundNum <= maxRound; roundNum++) {
        ...
        if (txt) {
          ...
          // FORK: prefer the persona/character or group name (e.g. "Iris") over the
          // model route label, matching the standard single-bubble path. Without
          // this, agent/tool-using replies showed the raw model id while plain
          // replies showed "Iris".
          let _agentRole = modelRouteLabel(
            pair.requestedModel,
            contModel,
