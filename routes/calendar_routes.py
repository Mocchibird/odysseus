    ...
    *,
    ...
    for comp in cal_data.walk():
        ...
        if source_uid:
            ...
            if existing:
                # An import predating the clamp below may have stored this same
                # event with a non-positive duration, which the list_events
                # overlap filter hides. Re-importing lands here and would skip
                # without touching that row, so the event would stay invisible.
                # Backfill the clamp onto the stored row before skipping it.
                # (Ported from upstream's import path.)
        ...
        # Clamp non-positive durations (DTEND <= DTSTART, e.g. exporters that
        # treat all-day DTEND as inclusive) so the row isn't hidden by the
        # list_events overlap filter. Ported from upstream's import path.
...
def setup_calendar_routes(upload_handler=None) -> APIRouter:
    ...
    async def sync_caldav_endpoint(request: Request, direction: str = "pull"):
        """Sync events with the configured CalDAV server and pull persistent
        ICS feeds. Returns counts + any per-calendar errors. Called by the
        frontend on calendar open and by the periodic scheduler loop."""
        owner = _require_user(request)
        from src.caldav_sync import sync_caldav_direction
        caldav = await sync_caldav_direction(owner, direction)
        # ICS subscriptions are read-only feeds — they only pull. On a
        # push-only sync there is nothing to combine, so return as-is.
        if (direction or "pull").strip().lower() == "push":
            return caldav
