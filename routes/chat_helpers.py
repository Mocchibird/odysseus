...
def _enforce_chat_privileges(request, sess) -> None:
    ...
    # FORK: admin-set GLOBAL chat/agent allowlist, applied to non-admins who have
    # no per-user allowed_models list. Enforced here at send-time so it can't be
    # bypassed by PATCHing the session model or hitting the API directly — the
    # picker filter is only cosmetic. Admins are never limited. A per-user list
    # (or block_all_models, which returns an empty frozenset rather than None)
    # already restricts, so this only fills the unrestricted case.
    if allowed_models is None:
        ...
        if not _is_adm:
            ...
            if isinstance(_g_allow, list) and _g_allow:
                allowed_models = frozenset(m for m in _g_allow if isinstance(m, str))
...
async def build_chat_context(
    ...
    #
    # FORK: incognito goes through add_user_message with incognito=True, which
    # adds to IN-MEMORY history (so the turn still has conversation context) while
    # skipping the parts that persist — see its docstring. Upstream instead routes
    # incognito to a request-local transcript store via _append_incognito_message,
    # a helper this fork removed; taking upstream's call site here would be a
    # dangling reference. Keep the fork's path, honour upstream's new persist gate.
    ...
    # FORK: per-user Iris language/persona variant. The shared custom slot is
