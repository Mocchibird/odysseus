...
from src.constants import OLLAMA_DEFAULT_PORT, LOCAL_HOSTS, COOKBOOK_STATE_FILE
...
def setup_model_routes(model_discovery):
    ...
    def _fetch_models(owner: str = "", is_admin: bool = False):
        ...
        for ep in endpoints:
            ...
            if ep_model_type in ("stt", "tts"):
                continue
            # Build the correct chat URL for this endpoint.
    ...
    def list_endpoint_models(
        ...
        try:
            ...
            # Hoisted out of the `if refresh:` below: the picker call after that
            # block needs both on every path, refresh or not.
            base = _normalize_base(ep.base_url)
            kind = _effective_endpoint_kind(ep, base)
            if refresh:
                category = _classify_endpoint(base, kind)
