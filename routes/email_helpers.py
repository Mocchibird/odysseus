...
def _send_smtp_message(cfg: dict, from_addr: str, recipients: list[str], message: str | bytes, timeout: int = 30) -> None:
    ...
    # Fork's connection helper handles ssl/starttls AND the local Proton-bridge
    # unverified-TLS case; upstream's _auth_smtp adds OAuth (XOAUTH2) alongside
    # password auth. Use both.
...
def _open_imap_connection(host: str, port: int, *, starttls: bool, timeout: int = _IMAP_TIMEOUT_SECONDS):
    ...
    if starttls:
        ...
        try:
            ...
            # Local mail bridge (e.g. Proton Bridge) serves a self-signed cert.
            # Close the plain socket first (#3174); for a local-bridge host retry
            # with an unverified TLS context, otherwise propagate.
            try:
                conn.shutdown()
            except Exception:
                pass
