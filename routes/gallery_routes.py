...
def setup_gallery_routes() -> APIRouter:
    ...
    async def gallery_replace(request: Request, image_id: str):
        ...
        try:
            ...
            # NOTE: the actual write happens off the event loop in
            # _write_and_probe() below (fork perf fix) — don't write inline here.
    ...
    def _get_or_404_album(db, album_id: str, user):
        ...
        if not _verify_owner(album, user):
            raise HTTPException(404, "Album not found")
        return album
