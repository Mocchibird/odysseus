...
#
# v2: upstream merge added _migrate_add_gallery_caption_column() and
...
def init_db():
    ...
    # Fork-only migration fast-skip: once a DB is stamped at SCHEMA_VERSION the
    # (idempotent but not free) migration block below is skipped on every boot.
    # Must sit AFTER the chmod hardening above, which has to run every start.
