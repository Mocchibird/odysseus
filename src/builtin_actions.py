...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        def _urgency_reminder_payload(reminder_keys):
            ...
            title = (_t18("urgent_email_title_one", _lang) if total == 1
                     else _t18("urgent_email_title_many", _lang, n=total))
            # A real listing — subject / sender / reason per urgent message —
            # so the reminder says WHICH mail to act on, not just "4 needing
            # reply". Deep-links when `app_public_url` is set in Settings.
            ...
            lines = [
                (_t18("urgent_email_lead_one", _lang) if total == 1
                 else _t18("urgent_email_lead", _lang, n=total)), "",
            ]
            for i, (key, value) in enumerate(sorted_urgent, 1):
                subj = (value.get("subject") or _t18("no_subject", _lang))[:160]
            ...
            if total > len(sorted_urgent):
                lines.append("")
                lines.append(_t18("and_n_more", _lang, n=total - len(sorted_urgent)))
            return title, "\n".join(lines)
...
async def action_morning_routine(owner: str, **kwargs) -> Tuple[str, bool]:
    """Start-of-day kickoff: morning digest + carry overdue reminders forward.
    Combines daily_brief with carry-forward so one trigger gets the day going."""
    try:
