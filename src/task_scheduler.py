...
class TaskScheduler:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        #
        # Resolve the zone via the DISPLAY resolver (crew zone, else the owner's
        # browser-learned timezone) — not _resolve_task_timezone — so pings that
        # fire while the browser is closed still reason in the user's local clock.
        tz_name = _resolve_task_display_tz(db, task)
        try:
