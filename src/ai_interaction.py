    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(connect=30.0, read=300.0, write=30.0, pool=30.0)) as client:
            ...
            if img.get("b64_json"):
                ...
                # Validate first (SSRF guard, upstream), then download via the
                # ASYNC client so a slow image host doesn't block the event loop
                # (and every other request) for up to 60s.
                result_url = img["url"]
                ...
                try:
                    # Run the (blocking) download off the event loop so a slow
                    # image host doesn't stall every other request for up to 60s.
