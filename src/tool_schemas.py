...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_health",
            "description": "Create, log and query the user's health/habits/training (same data shown in the Health panel). This IS the habit tracker — NOT a checklist note and NOT a vault file. Use for 'start a meditation habit' (create_habit), 'rename a habit / give it a 🧘 emoji / change its category or color' (update_habit), 'delete a habit' (delete_habit), 'mark meditation done' (check_habit), 'log my lunch ~600 kcal' (log_meal), 'fix that meal to 700 kcal / change its protein' (update_meal — get the meal id from action=calories), 'delete that meal' (delete_meal), 'I weigh 72kg' (log_weight), 'set my height/goal' (set_profile), 'calories today', 'weight progress'. Calories/weight feed the charts; habit check-ins fill the GitHub-style heatmap. To check off a habit that doesn't exist yet, create_habit first.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                               "enum": ["create_habit", "update_habit", "delete_habit", "check_habit", "list_habits", "habit_heatmap",
                                        "log_meal", "update_meal", "delete_meal", "log_weight", "log_training", "calories",
                                        "weight_trend", "set_profile", "summary"],
                               "description": "What to do."},
                    "name": {"type": "string", "description": "New habit name (create_habit). For update_habit/delete_habit/check_habit you may instead identify the target via 'habit'."},
                    "new_name": {"type": "string", "description": "New name when renaming an existing habit (update_habit)."},
                    "icon": {"type": "string", "description": "Emoji/icon for the habit (create_habit or update_habit), e.g. '🧘'."},
                    "color": {"type": "string", "description": "Optional accent color for the habit (create_habit or update_habit), e.g. a hex like '#7ec9a3'."},
                    "category": {"type": "string", "description": "Habit category (create_habit or update_habit)."},
                    "cadence": {"type": "string", "description": "Habit cadence: daily | weekdays | weekends (create_habit or update_habit). Default daily."},
                    "description": {"type": "string", "description": "Meal description (log_meal / update_meal)."},
                    "meal_id": {"type": "integer", "description": "Id of a logged meal to edit or remove (update_meal / delete_meal). Get it from action=calories, which lists each meal with its #id."},
                    "kcal": {"type": "integer", "description": "Calories for the meal (log_meal / update_meal)."},
                    "protein_g": {"type": "number", "description": "Optional protein grams (log_meal / update_meal)."},
                    "carbs_g": {"type": "number", "description": "Optional carbohydrate grams (log_meal / update_meal)."},
                    "fat_g": {"type": "number", "description": "Optional fat grams (log_meal / update_meal)."},
                    "sugar_g": {"type": "number", "description": "Optional sugar grams (log_meal / update_meal)."},
                    "kg": {"type": "number", "description": "Body weight in kilograms (log_weight)."},
                    "habit": {"type": "string", "description": "Existing habit name or id (check_habit/habit_heatmap/update_habit/delete_habit)."},
                    ...
                    "kcal_burned": {"type": "integer", "description": "Estimated calories burned in the session (log_training)."},
                    "summary": {"type": "string", "description": "Training notes (log_training)."},
                    "height_cm": {"type": "number", "description": "Height in cm (set_profile)."},
                    "date_of_birth": {"type": "string", "description": "YYYY-MM-DD (set_profile; for age in TDEE)."},
                    "sex": {"type": "string", "enum": ["M", "F"], "description": "Biological sex for BMR (set_profile)."},
                    "activity_level": {"type": "string", "description": "sedentary|lightly_active|moderately_active|very_active|extra_active (set_profile)."},
                    "target_kg": {"type": "number", "description": "Goal weight (set_profile)."},
                    "target_weekly_loss_kg": {"type": "number", "description": "Desired weekly loss for the calorie deficit (set_profile)."},
                    "daily_kcal_target": {"type": "integer", "description": "Manual calorie target override (set_profile)."},
                    "date": {"type": "string", "description": "YYYY-MM-DD; defaults to today (calories/check_habit). Pass yesterday's date to check_habit to mark a habit done for yesterday."},
