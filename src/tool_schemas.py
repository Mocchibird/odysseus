...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_health",
            "description": "Create, log and query the user's health/habits/training (same data shown in the Health panel). Use for 'start a meditation habit' (create_habit), 'mark meditation done' (check_habit), 'log my lunch ~600 kcal' (log_meal), 'I weigh 72kg' (log_weight), 'set my height/goal' (set_profile), 'calories today', 'weight progress'. Calories/weight feed the charts; habit check-ins fill the GitHub-style heatmap. To check off a habit that doesn't exist yet, create_habit first.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                               "enum": ["create_habit", "check_habit", "list_habits", "habit_heatmap",
                                        "log_meal", "log_weight", "log_training", "calories",
                                        "weight_trend", "set_profile", "summary"],
                               "description": "What to do."},
                    "name": {"type": "string", "description": "New habit name (create_habit)."},
                    "icon": {"type": "string", "description": "Optional emoji for the habit (create_habit), e.g. '🧘'."},
                    "category": {"type": "string", "description": "Optional habit category (create_habit)."},
                    "cadence": {"type": "string", "description": "Habit cadence: daily | weekdays | weekends (create_habit). Default daily."},
