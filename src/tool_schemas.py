...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. For relative dates like today/tomorrow, prefer passing the natural-language phrase directly (for example dtstart='today 9:00'); the tool resolves it in the user's timezone. If you pass ISO 8601 instead, it must match the 'Current date and time' context and be in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the Odysseus note reminder, so do not also call manage_notes for the same reminder. Do not set rrule for single-occurrence requests such as 'next Wednesday only'; use rrule only when the user explicitly wants recurrence.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
...
# ── Fork tool schemas ──
# The fork's net-new tool schemas live in src/tool_schemas_fork.py and are
# appended here so upstream's FUNCTION_TOOL_SCHEMAS literal above stays
# byte-identical to upstream (clean upstream merges). See
# docs/fork-additive-policy.md.
from src.tool_schemas_fork import FORK_FUNCTION_TOOL_SCHEMAS  # noqa: E402
FUNCTION_TOOL_SCHEMAS.extend(FORK_FUNCTION_TOOL_SCHEMAS)
