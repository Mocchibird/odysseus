...
- Files, Books & Gallery — `search_files`: search the user's content (uploaded files, books, authored docs) and cite the source so they can open + verify it. To STORE a file the user attached in chat, use `manage_files` {"action":"add","upload_id":...} with the upload_id from the message's attachment context — it ROUTES by type: images/videos to the Gallery (add `"album":"<name>"` to file them, e.g. game screenshots into a '<game>' album), PDFs/EPUBs to Books, everything else to the Files store. Never write_file for user content. To MANAGE existing media use `manage_gallery` (tag / rename / create album / sort into albums / favorite / hide / delete photos & videos — action=list to find ids); to manage existing files use `manage_files` (edit / append / retag / rename / delete).
...
NEVER use this to save user content/attachments — a user's uploaded file
goes through manage_files (action=add), which routes it to the Gallery /
Books / Files store by type.""",
...
Bulk delete/archive/mark emails. Use this for "delete all those" after listing emails. Pass the exact UIDs and the same account from the list result, then report only the tool result.""",
    ...
    "manage_calendar": """\
```manage_calendar
{"action": "create_event", "summary": "<event title>", "dtstart": "today 9:00"}
```
...
For relative dates like "today" or "tomorrow", pass the natural-language phrase directly (for example `"today 9:00"`), rather than inventing an ISO date yourself. The tool resolves it against the user's timezone. `dtstart` also accepts ISO ("2026-05-12T13:00:00") when the user gave an explicit date. \
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "ask_user": "- ```ask_user``` — Ask the user a multiple-choice question when the task is genuinely ambiguous and the answer changes what you do next (pick an approach, confirm an assumption, choose a target). Args (JSON): {\"question\": \"...\", \"options\": [{\"label\": \"...\", \"description\": \"...\"?}, ...], \"multi\": false?}. 2-6 options. The user gets clickable buttons; calling this ENDS your turn and their choice comes back as your next message. Prefer sensible defaults — only ask when you truly can't proceed well without their input. CRITICAL: once you've asked, the user's NEXT message IS their selected answer — act on it and continue the task. NEVER re-ask the same question or call ask_user again for a choice you already offered, even if your earlier question now appears as plain text (or as an `Options: …` line) in the history — that line IS your earlier multiple-choice question; you already asked it.",
...
def _build_system_prompt(
    ...
    try:
        ...
        _lang = "en"
    cache_key = (frozenset(disabled_tools or []), bool(mcp_mgr), needs_admin, _rt_key, compact, _ov_sig, owner, suppress_local_context, suppress_skills, _lang)
    ...
    # Workspace coding mode. Upstream's caller passes `workspace` through; without
    # this the parameter would be accepted and silently ignored, so a workspace
    # turn would lose its coding-mode rules. The fork has no _local_computer_rules
    # counterpart, so only the workspace branch applies here.
...
def _build_base_prompt(
    ...
    # Integration descriptions and MCP tool descriptions are user-editable
    # (an MCP server or integration config could smuggle prompt-injection
    # text), so they're computed and wrapped via untrusted_context_message
    # in _build_system_prompt instead of being appended to agent_prompt
    # here — same SECURITY reasoning as the skill index above. Do not
    # re-add a direct agent_prompt += injection for them.
    try:
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, block in enumerate(tool_blocks):
            ...
            if "ask_user" in result:
                ...
                # Persist the question AND the offered option labels into the
                # replayed assistant text. tool_event metadata is dropped on
                # replay, so a bare question reads like plain text the model
                # "forgot to ask via the tool" — it then loops and re-asks after
                # the user picks. Recording the options marks this unambiguously
                # as a multiple-choice prompt the user has now answered.
                _auq_labels = []
                for _o in (_auq.get("options") or []):
                    _lab = str((_o.get("label") if isinstance(_o, dict) else _o) or "").strip()
                    if _lab:
                        _auq_labels.append(_lab)
                _auq_parts = []
                if _auq_q and _auq_q not in full_response:
                    _auq_parts.append(_auq_q)
                if _auq_labels and "Options:" not in full_response:
                    _auq_parts.append("Options: " + " · ".join(_auq_labels))
                if _auq_parts:
