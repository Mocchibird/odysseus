...
# ── FORK TOOLS ──────────────────────────────────────────────────────────────
# Upstream's registry covers upstream's tool set. These five are the fork's and
# must be registered too, or the capability gate treats them as unknown (and
# tests/test_external_context_tool_gate.py fails the completeness assertion).
# Effects are chosen conservatively — over-restricting a tool degrades a feature,
# under-restricting one weakens the external-context gate.
_register(
    {"search_files"},
    ToolEffect.READ_PRIVATE,
    # Returns names/snippets from user-uploaded files.
    result_integrity=ResultIntegrity.EXTERNAL_UNTRUSTED,
)
_register(
    {"manage_files", "manage_gallery", "manage_health"},
    ToolEffect.READ_PRIVATE,
    ToolEffect.WRITE_PRIVATE,
    # These list/read as well as mutate, and echo user-supplied filenames,
    # captions and health notes back into the model's context.
    result_integrity=ResultIntegrity.EXTERNAL_UNTRUSTED,
)
_register(
    {"send_ping"},
    # Leaves the machine: delivers a notification to the user's ntfy topic.
    ToolEffect.EXTERNAL_SIDE_EFFECT,
)
