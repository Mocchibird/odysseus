    ...
    try:
        if action in ("list", "search", "find"):
            ...
            note = Note(
                ...
                # The tool's `due_date` param is the reminder time (fires a
                # notification), so store it as reminder_at — not a deadline.
                reminder_at=due_iso,
                source="agent",
            ...
            if args.get("due_date") is not None:
                ...
                try:
                    from routes.calendar_routes import parse_due_for_user as _pdt_user
                    # The `due_date` param is the reminder time → reminder_at.
