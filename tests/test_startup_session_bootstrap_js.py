...
_IMPORT_REWRITES = {
    ...
    "import markdownModule from './markdown.js';": (
        "import markdownModule from './markdown.mjs';"
    ),
    ...
    "import themeModule from './theme.js?v=397';": "import themeModule from './theme.mjs';",
...
const markdownModule = {
  ...
  processWithThinking: (text) => String(text ?? ''),
  squashOutsideCode: (text) => String(text ?? ''),
};
...
let fetchCount = 0;
const _isSessionsCall = (url) => String(url ?? '').includes('/api/sessions');
globalThis.fetch = async (url) => {
  fetchCount += 1;
  if (!_isSessionsCall(url)) {
    // e.g. /api/default-chat, which loadSessions() also consults.
