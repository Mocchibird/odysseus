...
const STUBS = new Map([
  [
    // FORK: settings.js imports PROMPT_TEMPLATES from presets.js. Upstream's
    // settings.js does not, so this smoke harness had no entry for it and threw
    // "Unexpected real module". Stubbed rather than loaded for real — the
    // coordinator smoke only exercises the settings shell.
    path.join(JS, 'presets.js'),
    { PROMPT_TEMPLATES: {} },
  ],
  [
    path.join(JS, 'ui.js'),
