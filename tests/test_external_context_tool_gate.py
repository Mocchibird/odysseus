...
def _patch_agent_loop(monkeypatch, round_responses, executed):
    ...
    # The fork bundles a published skill (quiz-spoiler-markdown), and skills are
    # injected as untrusted context that ARMS the post-external tool gate, so on
    # this fork every tool start would need an approval and this scenario could
    # never run. Upstream's CI ships no skills, hence the difference. Suppress the
    # lookup so the test exercises what it is about.
    ...
    monkeypatch.setattr(_skills_mod.SkillsManager, "get_relevant_skills", lambda self, *a, **k: [], raising=False)
    monkeypatch.setattr(_skills_mod.SkillsManager, "index_for", lambda self, *a, **k: [], raising=False)
    monkeypatch.setattr(
        agent_loop,
...
def test_frontend_tool_approval_uses_opaque_id_and_fixed_decisions():
    ...
    # FORK: numeric cascade token instead of upstream's date+feature string.
    import re as _re
    _v = _re.search(r"app\.js\?v=(\d+)", index).group(1)
    assert index.count(f"app.js?v={_v}") == 2
    ...
    # FORK: upstream stamps ONE shared token across the approval cluster; this
    # fork versions per module, bumping a module's own token only when that module
    # changes (theme.js?v=397, notes.js?v=526, ...). Upstream's "every module
    # carries the same token" form therefore cannot hold here.
    #
    # The invariant that still matters — and the bug the original guarded — is
    # that no module is referenced under TWO different tokens: a differing ?v=
    # is a different URL, so the browser instantiates the module twice and its
    # state exists in two copies.
    import re as _re2
    seen: dict[str, set[str]] = {}
    for source in approval_module_sources:
        for mod, tok in _re2.findall(r"([\w./-]+\.js)\?v=([\w]+)", source):
            seen.setdefault(mod.split("/")[-1], set()).add(tok)
    split = {m: sorted(t) for m, t in seen.items() if len(t) > 1}
    assert not split, f"module(s) referenced under two cache-bust tokens (loads twice): {split}"
...
def test_authorized_document_stream_precedes_completed_update(monkeypatch):
    ...
    # injected as untrusted context that ARMS the post-external tool gate, so on
    # this fork every tool start would need an approval and this scenario could
    # never run. Upstream's CI ships no skills, hence the difference.
    import services.memory.skills as _skills_mod
    monkeypatch.setattr(_skills_mod.SkillsManager, "load", lambda self, owner=None: [], raising=False)
    monkeypatch.setattr(_skills_mod.SkillsManager, "get_relevant_skills", lambda self, *a, **k: [], raising=False)
    monkeypatch.setattr(_skills_mod.SkillsManager, "index_for", lambda self, *a, **k: [], raising=False)
